...
# Cambridge Exam - SCPアップロードスクリプト
# 
# 使い方:
#   1. このスクリプトを編集して、サーバー情報を設定
#   2. chmod +x upload-via-scp.sh で実行権限を付与
#   3. ./upload-via-scp.sh を実行
...
# 必要なファイルのみコピー
cp -r ./*.php "$TMP_DIR/" 2>/dev/null
cp -r ./components "$TMP_DIR/" 2>/dev/null
cp .htaccess "$TMP_DIR/" 2>/dev/null
cp README-PHP.md "$TMP_DIR/" 2>/dev/null
cp QUICKSTART-PHP.md "$TMP_DIR/" 2>/dev/null

# SCPでアップロード
scp -P "$SSH_PORT" -r "$TMP_DIR"/* "$SERVER_USER@$SERVER_HOST:$SERVER_PATH/"
